...
namespace VenusSimulator
{
   internal sealed class MainViewModel : ViewModelBase
   {
      ...
      private readonly ImageDetector _detector = new();
      ...
      public RelayCommand LoadOperationsCommand => _loadOperationsCommand ??= new RelayCommand( () =>
      {
      ...
      public RelayCommand SaveOperationsCommand => _saveOperationsCommand ??= new RelayCommand( () => OperationSerializer.SerializeOperations( Operations.ToList() ) );

      private RelayCommand _createOperationCommand;
      public RelayCommand CreateOperationCommand => _createOperationCommand ??= new RelayCommand( () =>
      {
      ...
      public RelayCommand<MatchOperation> DeleteOperationCommand => _deleteOperationCommand ??= new RelayCommand<MatchOperation>( x =>
      {
         _ = Operations.Remove( x );
         _detector.RemoveTemplate( x.TemplateId );
      } );

      private RelayCommand<MatchOperation> _increasePriorityCommand;
      public RelayCommand<MatchOperation> IncreasePriorityCommand => _increasePriorityCommand ??= new RelayCommand<MatchOperation>( x =>
      {
      ...
      public RelayCommand StartCommand => _startCommand ??= new RelayCommand( () =>
      {
      ...
      private void OnStopSignalDetected( object sender, EventArgs e ) => _ = Application.Current.Dispatcher.Invoke( () => _window.WindowState = WindowState.Normal );
